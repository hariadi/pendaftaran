...
@section('page-header')
    <h1>
		...
		<small class="hidden-print">
			<a href="{{ route('admin.report.cert', ['id' => $event->id, 'action' => 'print']) }}" class="btn btn-info btn-sm" role="button"><i class="fa fa-print"></i> Cetak</a>
			<a href="{{ route('admin.report.cert', ['id' => $event->id, 'action' => 'xls']) }}" class="btn btn-info btn-sm" role="button"><i class="fa fa-file-excel-o"></i> Excel</a>
		</small>
	...
	<div class="row">
		<div class="col-xs-12">
			<h2 class="page-header">
				...
				<small class="pull-right">{{ $event->start_at->formatLocalized('%d %B %Y') }} - {{ $event->end_at->formatLocalized('%d %B %Y') }}</small>
			</h2>
